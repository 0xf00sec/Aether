...
void flatline_flow(uint8_t *code, size_t size, flowmap *cfg, chacha_state_t *rng) {
    ...
    /* Track cumulative expansion offset */
    size_t cumulative_expansion = 0;
    
    for (size_t i = 0; i < np; i++) {
        patch_t *p = &patch[i];
        size_t src = p->off + cumulative_expansion;  /* Adjust for previous expansions */
            ...
            case 3: /*  Jcc rel8 (opcode 70-7F) */
                ...
                if (new_disp >= -128 && new_disp <= 127 && src + 1 < buf_sz) {
                    ...
                    if (src + 6 <= buf_sz && out + 4 <= buf_sz) {
                        ...
                        /*  Shift everything after this instruction forward by 4 bytes */
...
void shuffle_blocks(uint8_t *code, size_t size, void *rng) {
    ...
    size_t expansion_offset = 0;
    for (size_t i=0; i<num_patches; i++) {
        info_t *p = &patches[i];
        size_t inst_off = p->inst_off + expansion_offset;
        int typ = p->typ;
        size_t tgt = p->tgt;
            ...
            if (typ==1||typ==2) { 
                ...
                if (d>=-128 && d<=127) {
                    ...
                    if (typ==3) {
                        ...
                        memmove(nbuf+inst_off+6, nbuf+inst_off+2, out+expansion_offset-inst_off-2);
                        nbuf[inst_off]=0x0F;
                        ...
                        /* JMP rel8 -> JMP rel32 */
                        memmove(nbuf+inst_off+5, nbuf+inst_off+2, out+expansion_offset-inst_off-2);
                        nbuf[inst_off]=0xE9;
                        rel=(int32_t)(tgt-(inst_off+5));
                        memcpy(nbuf+inst_off+1,&rel,4);
                        expansion_offset += 3;
