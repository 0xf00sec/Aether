...
static wipe_conf_t *prep_nuker(int passes) {
    ...
    for (int i = 0; i < passes; i++) cfg->patterns[i] = WIPE_RANDOM;
    cfg->passes = passes;
    cfg->custom = 0;
...
/* Fork child to wipe binary, parent exits immediately */
void k_ill(void) {
    char path[1024] = {0};
    uint32_t sz = sizeof(path);
    if (_NSGetExecutablePath(path, &sz) != 0) exit(EXIT_FAILURE);
    ...
    if (pid == 0) {
        usleep(500000);
        if (kill(parent_pid, 0) != 0) _self(path);
        exit(EXIT_SUCCESS);
    }
